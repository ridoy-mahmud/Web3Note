...
import { Pin, Archive, Trash2, RotateCcw, Clock } from 'lucide-react';
...
const NoteCard = ({ note, onUpdate, onTrash, onRestore, onPurge, onClick }: NoteCardProps) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      {note.pinned && (
        <Pin className="w-4 h-4 text-primary mb-2 rotate-45" aria-label="Pinned" />
      )}

      {note.title && (
        <h3 className="font-bold text-foreground text-base mb-2 tracking-tight">{note.title}</h3>
      )}
      ...
      {note.type === 'checklist' && note.items.length > 0 && (
        <div className="mb-3">
          ...
          />
          {note.items.length > 5 && (
            <p className="text-xs text-muted-foreground mt-1">+{note.items.length - 5} more</p>
          )}
          <div className="mt-2 flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-muted-foreground font-medium">{completedCount}/{totalCount}</span>
          </div>
        </div>
      )}
      ...
      {note.images && note.images.length > 0 && (
        <div className="mb-3">
          <ImageAttachments images={note.images.slice(0, 4)} onChange={() => {}} readonly compact />
        </div>
      )}
      ...
      {note.codeBlocks && note.codeBlocks.length > 0 && (
        <div className="mb-3 space-y-2">
          ...
          {note.codeBlocks.length > 1 && (
            <p className="text-[11px] text-muted-foreground">+{note.codeBlocks.length - 1} more code blocks</p>
          )}
        </div>
      )}
      ...
      <div
        className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150 pt-2 border-t border-border"
